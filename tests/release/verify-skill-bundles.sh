...
unzip -p "$output_dir/everyline-cli-skill.zip" SKILL.md | grep -F '统一默认 `prod` 环境' >/dev/null
unzip -p "$output_dir/everyline-cli-skill.zip" SKILL.md | grep -F '当前 Profile 是 dev、test 或 blue 时不得继承它' >/dev/null
...
unzip -p "$output_dir/everyline-cli-skill.zip" SKILL.md | grep -F 'EVERYLINE_DOUBAO_SKILLS_DIR' >/dev/null
