...
智审开放平台 Go 命令行客户端。覆盖接口文档中的 25 个远端 operation 映射：Profile/tenant token、合同审查工作流、审查清单以及规则分组/规则管理；其中 21 项已开放真实调用，4 项缺字段级详情的批量写操作失败关闭。文件快照和字段捷径接口按当前 CLI 范围暂不纳入同步。
