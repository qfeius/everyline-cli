...
- 等待期间只称任务已创建或正在处理。
- 成功时按 `SKILL.md` 的“成功结果输出”统一展示基础信息表、审查概览、上下排列的两个图表以及末尾两行；不展示服务端原始终态对象。
- 基础信息来自本次实际文件与已确认参数，风险统计及主要问题来自真实结果。上方为风险等级分布环形图（数量、占比），下方为按数量降序排列的风险类别横向条形图（每类数量）；缺失、零值及类别重叠遵循公共输出规范。
- 详情入口优先使用宿主原生链接按钮，文字为“查看详情”；不支持时输出 `审查结果：[查看详情](<REVIEW_DETAIL_URL>)`，把 `reviewDetailUrl` 当作不可拆分原始字符串逐字写入 Markdown 链接目标。URL 内实际存在的 `id/version/source/businessId/taskId/entry/appType/token` 等 query 参数及原始顺序和编码必须保留，但不得在链接之外单独展示、解释或罗列。
- 不展示 `taskId`、`businessId`、`fileId`、`fileHash`、`id`、`status`、终态枚举、轮询参数、request ID、CLI 命令、退出码或其他服务端参数。链接文字固定为“查看详情”，链接目标必须与字段值逐字一致；不使用省略号、星号、占位符、短链接或重拼地址。
- 状态成功但链接缺失时，仅在“审查结果”一项说明链接缺失，不拼接地址；没有风险数据时在概览和对应图表位置说明未返回可展示的数据，不猜测数量。
- 末尾固定文案为“有效期提示：审查结果详情链接默认有效期为两小时，请及时查看。”。审查结果和有效期提示各占一行，标题与内容同行，标题不加粗。
- 即使同轮执行了延迟更新，也不把更新状态或诊断追加到成功审查回复；成功回复保持统一结构。
