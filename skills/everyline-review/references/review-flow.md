...
- 等待期间只称任务已创建或正在处理。
- 成功时按 `SKILL.md` 的“成功结果输出”统一展示基础信息表、审查概览以及末尾两行；不展示服务端原始终态对象。
- 基础信息来自本次实际文件与已确认参数，风险统计及主要问题来自真实结果。不展示图表；缺失、零值及未知等级遵循公共输出规范。
- 详情入口统一输出 `审查结果：[查看详情](<REVIEW_DETAIL_URL>)`，把 `reviewDetailUrl` 当作不可拆分原始字符串逐字写入 Markdown 链接目标。URL 内实际存在的 `id/version/source/businessId/taskId/entry/appType/token` 等 query 参数及原始顺序和编码必须保留，但不得在链接之外单独展示、解释或罗列。
...
- 状态成功但链接缺失时，仅在“审查结果”一项说明链接缺失，不拼接地址；没有风险数据时在概览中说明未返回可展示的数据，不猜测数量。
