...
- Codex 当前回合暴露原生结构化选项工具（如 `request_user_input`）时，与 WorkBuddy 原生选择组件一样优先使用；每次选项卡只收集一个业务维度。用户已经明确给出且能唯一匹配的值直接复用。
- 立场方和审查强度属于单选，并且真实候选数量符合宿主限制时使用原生选项卡。选项只展示本次真实主体的 `name（role）` 或固定的弱势、中立、强势，不展示内部 ID。
- Codex 的 `request_user_input` 当前只表达互斥单选；审查清单需要多选、稳定全局编号、翻页和搜索时继续使用编号文字交互，不把多选拆成一串是/否问题，也不增加完成确认。
- 当前回合没有原生选项工具、候选超过组件容量，或组件会改变多选语义时，直接回退到编号文字交互；不为展示选项卡切换协作模式。
- 原生选项卡与编号文字交互必须共享同一份冻结候选映射；宿主返回的展示值映射回本次 CLI 查询中的同一候选，不能把选项序号或展示文本当作资源 ID。
...
4. 把真实清单按每页 4 个切分，页数按完整真实清单数量计算。每次清单交互都显示 `第 X/Y 页`，并展示当前页清单的稳定全局编号、名称和可用类型信息；明确提示用户可多选、需在一次回复中给出全部编号，选择后将直接进入立场方。
5. 每次清单交互只包含当前页清单与“上一页 / 下一页 / 按名称搜索”动作，不得提供额外的完成或确认动作，也不得同时放入立场方问题。首页可省略“上一页”，末页可省略“下一页”；动作不得占用或改变清单的全局编号。
6. 用独立会话状态保存当前页和完整的编号到真实清单 ID 映射。翻页只更新当前页，不产生清单选择；用户可在一次回复中选择一个或多个稳定全局编号，包括此前浏览页面中的编号。
7. 用户选择“按名称搜索”时，在下一次仅收集搜索文字，并在完整候选集合中匹配，不局限于当前页。用户在任意清单交互中直接输入未展示清单的完整名称时也执行全局匹配；唯一匹配时将其作为最终选择，多个匹配时展示类型、规则名称或全局编号供区分，无匹配时保留当前页码。
8. 用户一次回复中包含至少一个有效清单编号或唯一名称时，立即冻结本次全部选择，将真实清单统一转换为完整查询得到的 ID，并在下一次独立交互直接进入立场方选择，不再要求“完成选择”或二次确认。允许组合选择多个真实清单，也允许内置规则包与真实清单组合；没有有效规则来源时继续清单阶段。
