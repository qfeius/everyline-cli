...
- WorkBuddy 的审查清单选择固定使用 `AskUserQuestion`，单个问题设置 `multiSelect=true`，每页把统一候选快照中的 2–4 项放入 `options`。WorkBuddy 不因审查清单需要多选或分页回退到编号文字交互，也不使用 Markdown 表格模拟选项组件；组件未就绪时保留冻结状态并报告宿主能力缺口。
- WorkBuddy 的立场方和审查强度使用 `AskUserQuestion` 且设置 `multiSelect=false`。清单页只有 1 个业务候选时加入一个当前可用的上一页或搜索导航项满足组件最少 2 项约束；正常翻页和搜索通过组件自由输入入口接收，导航项不映射为规则来源。
- Codex 当前回合暴露原生结构化选项工具（如 `request_user_input`）时，符合容量的立场方和审查强度使用互斥单选选项卡。Codex 的 `request_user_input` 当前只表达互斥单选，因此审查清单使用稳定编号文字协议；豆包同样使用编号文字协议。
- 原生选项卡与编号文字交互必须共享同一份冻结候选映射；宿主返回的展示值映射回本次 CLI 查询中的同一候选，不把选项序号或展示文本当作资源 ID；不为展示选项卡切换协作模式。
...
2. 把固定内置项 `0. 按合同类型自动匹配内置规则包` 放在首位，再按 CLI 返回顺序追加从 1 连续编号的真实自定义清单；内置规则包与真实自定义清单组成一份统一候选列表。读取结束后冻结完整列表、稳定编号及编号到规则来源的映射，展示编号与解析用户回复必须使用同一份映射；翻页、搜索和返回上一页都只读取这份快照。响应只有规则 ID 时，读取全部规则分组及规则页面，使用本次查询建立 ID 到规则名称的映射。
3. 统一设置 `pageSize = 4`、`totalCandidates = len(candidates)`、`pageCount = ceil(totalCandidates / pageSize)`，按当前页切片统一候选。内置项和真实清单共同计入每页容量，编号 0 只出现在第一页；6 个真实清单加内置项共 7 项时，第 1 页为 0、1、2、3，第 2 页为 4、5、6。
4. 用独立会话状态保存 `currentPage`、`pageSize`、`pageCount` 和完整编号映射。翻页只更新当前页，不产生清单选择；到达边界时保持合法页码，不重新查询或把全部候选改写成单页。
5. WorkBuddy 每次显示清单页都调用 `AskUserQuestion`，问题展示 `第 X/Y 页`，当前页候选写入 `options` 并设置 `multiSelect=true`。Codex 和豆包编号模式显示同一当前页和稳定全局编号；各宿主都不得同时放入立场方问题或增加完成确认。
6. 名称搜索作用于冻结后的完整统一候选列表。唯一匹配时将其作为最终选择，多项匹配时展示类型、规则名称或全局编号，无匹配时保留当前页码。
7. 用户可在一次文字回复或一次 WorkBuddy 多选答案中选择一个或多个稳定全局编号或唯一名称，包括此前浏览页面中的编号；重复项去重。编号 0 映射为 `matchContractTypeRulePackage=true`，其余编号映射为真实清单 ID 并写入 `selectedCheckListIds`。
8. 至少选中一种规则来源后立即冻结全部选择，并在下一次独立交互直接进入立场方选择，不再要求“完成选择”或二次确认。允许组合选择内置规则包和多个真实清单；没有有效规则来源时保留分页状态并继续清单阶段。

## 提取并匹配主体

清单选择冻结后，立即进入新的独立立场方阶段并使用上传结果调用：

```text
review subject extract --profile <profile> --as <identity> --business-id <businessId> --file-id <fileId> --file-hash <fileHash> --output json
```

- 立场方交互不得与任何清单页面或清单搜索动作合并到同一个宿主选择卡片。
