...
| ID | 状态 | 用户示例或条件 | Skill 处理 | 结束条件 |
| --- | --- | --- | --- | --- |
| SUBJECT-01 | 已支持 | 用户未提前说明立场方 | 调用主体提取，展示完整真实主体名称 | 用户选择一项 |
| SUBJECT-02 | 已支持 | 用户已说“站在示例采购公司立场”且唯一匹配 | 直接采用唯一候选 | 不重复询问 |
| SUBJECT-03 | 已支持 | 输入匹配多个主体 | 展示可区分候选项和编号 | 用户明确选择 |
| SUBJECT-04 | 已支持 | 输入没有匹配主体 | 展示本次提取得到的候选项 | 用户重新选择 |
| SUBJECT-05 | 受限 | CLI 未返回可用主体 | 不根据文件名、登录用户或甲乙方关系猜测 | 返回提取结果并停止 |
| SUBJECT-06 | 已支持 | 已确定唯一主体 | 同时写入内部 `selectedPosition` 和 `selectedAuditRole` | 不向用户追问第二个角色字段 |
