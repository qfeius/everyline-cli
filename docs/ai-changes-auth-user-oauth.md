...
- dev/test 环境预设补充 `contract-review` OAuth metadata、public client 和 `127.0.0.1:8000/login` 回调；blue/prod 不猜测未确认的 client 或 endpoint。
...
协议依据：`contract-cli` OAuth 实现、`contract/common-organization-v2` authorization server、`contract/open-platform` PKCE 集成和当前 dev/test 发布配置。
