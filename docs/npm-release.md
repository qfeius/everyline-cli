...
标签触发的唯一工作流先拒绝非 blue 版本或渠道，然后运行 Windows 打包检查、完整测试和安装校验、发布 GitHub 预发布制品、上传 npm 安装包和两项 Skill ZIP、通过 OIDC 发布 npm 包，最后回查 blue 渠道版本。发布始终使用 `--tag blue`，不覆盖 `latest` 或 `beta`。
