...
8. Skill 只查询该 task ID；等待成功后统一返回基础信息表、审查概览、静态 HTML/SVG 风险可视化，以及“查看详情”链接和两小时有效期提示，不展示 task ID、终态字段或其他服务端参数。风险等级环形图与风险类别条形图上下竖排；零风险时隐藏图表，宿主不支持渲染时降级为文字并在“说明”行标注。
...
说明：<可选，仅在未知等级、图表降级、结果不完整或数据缺失时输出>

问题主要集中在：<基于真实结果简洁概括>。

<此处渲染上下竖排的静态风险图表；风险项总数为 0 时隐藏图表，首行改为“未发现风险”>
...
| 最终结果 | 统一返回基础信息表、审查概览、静态风险可视化，以及指向完整签名 `reviewDetailUrl` 的“查看详情”链接和有效期提示；不展示 task ID、终态字段或其他服务端参数 |
...
`reviewDetailUrl` 是后端签发的用户可访问免登录链接。Skill 把字段值作为不可拆分字符串，从 `https://` 到最后一个查询参数逐字写入“查看详情”的 Markdown 链接目标，保留完整 `token`；不解析、脱敏、重新编码或改写。URL 中的参数不在链接之外单独展示，最终回复遵循基础信息表、审查概览、适用的风险可视化和末尾两行的统一格式。
