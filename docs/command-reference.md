...
`--env` 仅支持 blue 预设：业务地址 `https://open-b.qfei.cn`，自有用户认证页面 `https://contract-agent-b.qfei.cn`，token 地址 `https://open-b.qfei.cn/open-apis/auth/v3/tenant_access_token/internal`。`--env dev`、`--env test`、`--env prod` 均返回参数错误。blue 预设同时包含 `https://myaccount-b.qfei.cn/.well-known/oauth-authorization-server/contract-review` OAuth metadata、`contract-review` business type、`contract-review:full` scope 和 `http://127.0.0.1:8000/login` loopback 回调配置，不内置浏览器 client ID；Codex 本地每次显式 `auth login --as user` 都会读取 metadata 的 `registration_endpoint`，动态注册并保存本次返回的 client ID，替换 Profile 中的历史浏览器 client。blue 内置独立 Device client `zscli_bc60fee4de9913ae`，供豆包/WorkBuddy 使用。使用 `--env` 时不需要再传 `--base-url`/`--token-url`；未指定 `--env` 且未提供自定义地址时默认使用 blue；使用自定义地址时需同时传入这两个地址，可额外传 `--auth-url` 与 `--oauth-*` 参数。`app` 身份必须通过 `--app-id`、`EVERYLINE_APP_ID` 或 Profile 提供 app-id，并通过 `--app-secret`、`--app-secret-stdin`、环境变量或当前 Profile 的本地安全存储提供 app secret；macOS 优先使用 Keychain，失败时回退 `secrets.json`，其他系统使用该文件。只有显式 `--save-app-secret` 才会保存 secret。Codex 本地 user 使用浏览器 OAuth，豆包/WorkBuddy user 使用 `auth init`/`auth complete` Device Grant；`--no-open-browser` 只影响 Codex 浏览器登录。例如：`config add blue-user --env blue --default-identity user`。
...
`auth init`/`auth complete` 为豆包与 WorkBuddy 沙箱提供不依赖 `127.0.0.1` callback 的 Device Grant。blue 的 `contract-review` 业务使用独立 Device client `zscli_bc60fee4de9913ae` 和 `contract-review:full` scope；`auth init` 不动态注册 client，也不复用 Codex 的浏览器 client。自定义 Profile 使用 Device Grant 时，需通过 `--oauth-device-client-id` 提供平台确认的独立 client；已移除环境的 metadata 不再自动补全 Device client。豆包 AgentKit 使用工作区加密文件并要求 `EVERYLINE_CLI_CREDENTIAL_KEY_V1`（base64 编码 32 字节）；豆包工作任务按 `SESSION_ID` 派生会话隔离密钥；WorkBuddy 按 `CODEBUDDY_SESSION_ID` 使用系统 Credential Manager/Keychain/Secret Service。WorkBuddy 的 `auth init`、`auth complete` 和后续 `auth status` 必须显式复用同一个稳定值，通用 `SESSION_ID` 的变化不参与 WorkBuddy 凭证定位。`auth complete` 本地未找到事务时先恢复原标识并重试 `auth complete`，不直接重启授权。加密凭证中携带非敏感 Device Profile 快照，沙箱重建后可通过显式 `--profile` 恢复。OAuth metadata 尚未发布 `device_authorization_endpoint` 时，CLI 会明确报告服务端能力缺口；不会在远端沙箱自动回退到 loopback 登录。
