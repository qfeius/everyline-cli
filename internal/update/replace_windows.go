...
const (
	deferredReplacementAttempts = 20
	deferredReplacementDelay    = 250 * time.Millisecond
)
...
func replaceBinary(temporaryPath string, targetPath string) (bool, error) {
	...
	command := exec.Command(helperPath, "__everyline-cli-replace", temporaryPath, targetPath, helperPath)
	command.Stdin = nil
	command.Stdout = io.Discard
	// helper 在目标 EXE 解锁后才产生最终结果；继承 stderr 才能让异步成功或失败对用户可见。
...
// runDeferredReplacement 由独立 helper 在有限窗口内重试替换，避免依赖可能已退出的父进程句柄。
// 入参：args []string 依次包含制品临时路径、目标路径和 helper 自身路径。
// 返回值：error，参数或最终替换失败时非 nil。
func runDeferredReplacement(args []string) error {
	if len(args) != 3 {
		return errors.New("替换助手参数数量错误")
	}
	temporaryPath, targetPath, helperPath := args[0], args[1], args[2]
	...
	// 目标 EXE 仍运行时 rename 会失败；直接重试既覆盖文件锁，也覆盖父进程早于 helper 启动完成的竞态。
	return replaceWithRetry(temporaryPath, targetPath, deferredReplacementAttempts, deferredReplacementDelay, os.Rename, time.Sleep)
}
