...
- config.selectedPosition string（必填）：审查立场，填写合同主体精确名称。
- config.selectedAuditRole string（必填）：审查角色，填写同一主体的角色，例如甲方或乙方。
...
实际 HTTP 请求固定补入 usageReportContext.reportBusinessCode=everyLine_100_openApi_cli，该字段不属于 CLI 输入。`,
		Example: `  everyline-cli review task start --data '{"businessId":"biz-001","fileId":123,"fileHash":"<upload.fileHash>","config":{"selectedPosition":"xxx公司","selectedAuditRole":"甲方","reviewStrength":"中立","selectedCheckListIds":["2001001"],"matchContractTypeRulePackage":true}}' --dry-run
...
发起审查的实际 HTTP 请求固定补入 usageReportContext.reportBusinessCode=everyLine_100_openApi_cli，该字段不属于 RunSpec 输入。`,
		Example: `  everyline-cli review run --input review-run.json --output json
  everyline-cli review run --data '{"source":{"type":"file","path":"./contract.pdf","name":"合同.pdf"},"config":{"selectedPosition":"xxx公司","selectedAuditRole":"甲方","reviewStrength":"中立","selectedCheckListIds":["2001001"],"matchContractTypeRulePackage":true},"wait":true}' --dry-run`,
		Args: cobra.NoArgs,
		RunE: func(command *cobra.Command, args []string) error {
